...
def catalog(request):
	...
	return render(request, "instashop\catalog.html", ctx)

def cart(request):
	cart = request.session['cart']
	request.session.set_expiry(0)
	ctx = {'cart': cart, 'cart_size': len(cart), 'cart_items': cartItems(cart), 'total_price':priceCart(cart)}
	return render(request, "instashop\cart.html", ctx)
...
def checkout(request):
	...
	return render(request, "instashop\checkout.html", ctx)
...
def adminLogin(request):
	...
	return render(request, "instashop/admin_login.html", None)
